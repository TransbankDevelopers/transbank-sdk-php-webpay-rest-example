...
<div class="w-full max-w-none">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-8">
        ...
        <input id="deferred_commit_token" type="text" readonly value="{{ $token }}" class="w-full bg-white" />
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div class="bg-white border border-gray-200 rounded-lg p-4 h-full flex flex-col">
            ...
            <form class="tbk-form-stack tbk-form-stack-flex" method="post" action="{{ route($routeNames['capture']) }}">
                @csrf
                <div>
                    <label for="capture_token_ws">Token</label>
                    <input id="capture_token_ws" type="text" class="w-full" name="token_ws" value="{{ $token }}">
                </div>
                <div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 h-full flex flex-col">
            ...
            <form class="tbk-form-stack tbk-form-stack-flex" method="post" action="{{ route($routeNames['status']) }}">
                @csrf
                <div>
                    <label for="status_token">Token</label>
                    <input id="status_token" type="text" class="w-full" value="{{ $token }}" name="token">
                </div>
